...
function renderHome(){
  ...
  <main class="wrap">
    ...
    ${acc('12명의 공개 알리바이 · 목록 / 그래프 / 공간도', `<div class="seg"><button class="${view==='list'?'on':''}" onclick="setAlibiView('list')">목록</button><button class="${view==='graph'?'on':''}" onclick="setAlibiView('graph')">시간별 그래프</button><button class="${view==='floor'?'on':''}" onclick="setAlibiView('floor')">공간 구조도</button></div><div id="alibiwrap">${view==='graph'?alibiGraph():(view==='floor'?floorPlan():alibiList())}</div>`, false, 'acc-alibi')}
